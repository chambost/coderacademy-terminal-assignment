...
# female_say["Well if it isn't my favourite patient Mr. #{responses[0]}"]
# male_say["Thanks for seeing me at such short notice Dr. #{responses[1]}. So what's the news?"]
# female_say["Well we just finished processing your tests and unfortunately the results are #{responses[2]}"]
# male_say["#{responses[3]}"]
# female_say["I completely understand that reaction. Your blood pressure is #{responses[4]} over #{responses[5]}"]
# female_say["And our x-rays found a #{responses[6]} in your #{responses[7]}"]
# female_say["It also appears that you have a rare condition known as #{responses[8]} #{responses[9]}"]
# male_say["#{responses[10]}"]
# male_say["Is there any way to cure it?"]
# female_say["I'm going to write you a prescription for these pills. I want you to take one every #{responses[11]}"]
# male_say["But I heard those pills cause involuntary #{responses[12]}"]
# female_say["I'm afraid they do. Side effects also include an itchy #{responses[13]} and a decrease in the desire to #{responses[14]}"]
# male_say["What can I do to stay healthy?"]
# female_say["I would suggest waking up early every morning and doing stretches and then #{responses[15]} in the park"]
# male_say["That's great advice especially for a #{responses[16]} #{responses[17]} such as myself"]
# female_say["You know the best part about being a doctor is that I get to treat patients like you and then go home to my #{responses[18]} #{responses[19]} and then sing them my favourite song"]
# male_say["Which is?"]
# female_say["#{responses[20]}"]
...
dialogue.each do |d| 
    actor, words = d.split(":") 
    actor_to_voice[actor].call(words)
end